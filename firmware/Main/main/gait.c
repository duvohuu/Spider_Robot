...
// Tầm với ngang tư thế đứng — tính lại mỗi khi chiều cao thay đổi
// Công thức: L1 + sqrt(L2² - (z - L3)²)  (giữ tibia thẳng đứng)
static float g_stand_x = STAND_X;

static float compute_stand_x(float z)
{
    float dz = z - L3;
    float sq = L2 * L2 - dz * dz;
    return (sq > 0.0f) ? L1 + sqrtf(sq) : L1;
}
...
#define FOOT_FWD_X  (g_stand_x + g_step_x)
#define FOOT_BWD_X  (g_stand_x - g_step_x)
#define FOOT_Y      0.0f
...
#define LF_BX  ( g_stand_x * 0.7071f)
#define LF_BY  ( g_stand_x * 0.7071f)
#define LM_BX  ( 0.0f)
#define LM_BY  ( g_stand_x)
#define LB_BX  (-g_stand_x * 0.7071f)
#define LB_BY  ( g_stand_x * 0.7071f)
#define RF_BX  ( g_stand_x * 0.7071f)
#define RF_BY  (-g_stand_x * 0.7071f)
#define RM_BX  ( 0.0f)
#define RM_BY  (-g_stand_x)
#define RB_BX  (-g_stand_x * 0.7071f)
#define RB_BY  (-g_stand_x * 0.7071f)
